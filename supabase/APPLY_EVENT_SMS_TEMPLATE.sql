...
--
-- Source: migrations/082_event_sms_template.sql
-- Needed by: the SMS step in the wizard, which games with the
-- `sms_notifications` flag get after the rewards step. Until this runs, that
-- step cannot save a message (the write fails on a missing column) and every
-- game sends the built-in default text instead; a game without the flag never
-- sees the step and is untouched either way.
--
-- Goes with APPLY_PARTICIPANT_PHONE.sql (081), which is where the numbers this
-- message is sent to come from. Order between them does not matter.
