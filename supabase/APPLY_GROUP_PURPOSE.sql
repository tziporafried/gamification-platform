...
--
-- Source: migrations/090_group_purpose.sql
-- Needed by: group purpose - telling a group that competes apart from a group
-- that exists only so tasks, prizes and draws can be aimed at part of the
-- roster. Until this runs there is nowhere to record the difference, so the
-- choice cannot be saved and the leaderboard cannot leave anyone out.
--
-- Safe on a live event. Adds one defaulted column and replaces one function;
-- every existing group is 'competition', which is what it already was, and the
-- leaderboard returns exactly the same rows as before.
--
-- Does NOT create the `group_purpose` feature flag - that row is made by hand
