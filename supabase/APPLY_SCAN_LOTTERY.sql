...
--
-- Source: migrations/080_scan_lottery_rounds.sql
-- Needed by: the scan lottery (feature flag `scan_based_lottery`). Until this
-- runs, opening a scan lottery reports that the round could not be opened and
-- the flag's dock stays unusable; the points lottery is untouched either way,
-- and the offline player never reads it.
