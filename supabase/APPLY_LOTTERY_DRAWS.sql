...
--
-- Source: migrations/080_lottery_draws.sql
-- Needed by: the lottery history a programme manager reads - what was given
-- away, who was in the running, who won, how the pool was chosen. Until this
-- runs, a finished lottery is recorded only in the operator's own browser
-- (localStorage), which is what it has always done; nothing else breaks and
-- the offline player never reads it.
