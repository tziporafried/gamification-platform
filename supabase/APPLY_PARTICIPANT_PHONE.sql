...
--
-- Source: migrations/081_participant_phone.sql
-- Needed by: the phone number a game with the `sms_notifications` flag collects
-- for every participant - the field on the participants step, and the טלפון
-- column in the roster import. Until this runs, that game cannot store a number
-- (every save fails on a missing column); a game without the flag is untouched.
--
-- The flag itself is created by hand in the admin panel, like every other one.
-- Order does not matter: without the flag nothing asks for a phone, and without
-- this file the flag has nowhere to put one.
