 ...
 *
 * Counted by the database (migration 086). Counting them here instead - one
 * select per table, one row per participant and per scan - runs into
 * PostgREST's 1000-row response cap, and because the cap is per request rather
 * than per event it silently reports 0 for whichever events fall past it.
 * fallbackCounts below still does exactly that, paginated so the total is
 * right, for installations where 086 has not been applied yet.
 */
export async function fetchEventsPlayMeta(eventIds: string[]): Promise<Record<string, EventPlayMeta>> {
  ...
  if (error) return fallbackCounts(eventIds, meta)
