...
import { GROUP_COLUMN_HEADER, NAME_COLUMN_HEADER, PHONE_COLUMN_HEADER } from './rosterPlan.ts'
 ...
 * sold SMS must download the same two-column file it always did, with no
 * column, no header and no example number hinting at a feature it does not have.
 */

test('a game without the flag downloads the file it always did', () => {
  assert.deepEqual(rosterTemplateRows(), [
    [NAME_COLUMN_HEADER, GROUP_COLUMN_HEADER],
...
test('the groups-only template is unchanged too', () => {
  ...
  assert.deepEqual(rows[0], [NAME_COLUMN_HEADER])
...
test('the flag adds the phone column, and only then', () => {
  ...
  assert.deepEqual(rows[0], [NAME_COLUMN_HEADER, GROUP_COLUMN_HEADER, PHONE_COLUMN_HEADER])
...
test('a phone column with no groups keeps its own place', () => {
  ...
  assert.deepEqual(rows[0], [NAME_COLUMN_HEADER, PHONE_COLUMN_HEADER])
