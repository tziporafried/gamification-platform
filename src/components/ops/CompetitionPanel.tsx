...
function LeaderboardView({ rankedGroups, accent }: { rankedGroups: RankedGroup[]; accent: AccentRgb }) {
  ...
  if (rankedGroups.length === 0) {
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-lg font-black text-muted">ממתינים לסריקה הראשונה...</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex h-full flex-col gap-3 overflow-hidden">
      <p className="shrink-0 text-xs font-black uppercase tracking-widest text-muted">דירוג קבוצות</p>
          ...
          return (
            <motion.div
              ...
              className={cn(
                ...
                isFirst && 'border-warning bg-surface-elevated shadow-card',
                !isFirst && isTop3 && 'border-border bg-surface-elevated',
                !isTop3 && 'border-border',
                isFirst ? 'p-[14px_16px]' : isTop3 ? 'p-[10px_14px]' : 'p-[8px_12px]',
              )}
              ...
              <div className="flex items-center gap-3">
                <div className="w-10 shrink-0 text-center">
                  {medal ? (
                    ...
                    <div
                      className={cn(
                        'mx-auto flex h-7 w-7 items-center justify-center rounded-full text-sm font-black',
                        rankBadgeClass(g.rank),
                      )}
                      style={g.rank > 3 ? { background: `rgba(${ar},${ag},${ab},0.85)` } : undefined}
                    >
                      {g.rank}
                    </div>
                  )}
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1.5 flex items-center justify-between gap-2">
                    ...
                    <span
                      className={cn('shrink-0 tabular-nums font-black', isFirst ? 'text-warning' : 'text-foreground')}
                      style={{ fontSize: isFirst ? 20 : isTop3 ? 15 : 13 }}
                    >
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-border">
                    <motion.div
                      className={cn('h-full rounded-full', isFirst && 'bg-warning')}
                      style={{ background: isFirst ? undefined : g.group_color }}
                      animate={{ width: `${pct}%` }}
...
function ClosestBattleView({ rankedGroups }: { rankedGroups: RankedGroup[] }) {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-5 text-center" dir="rtl">
      <p className="text-xs font-black uppercase tracking-widest text-muted">קרב בראש הדירוג</p>

      <div>
        <div className="mb-2 flex items-center justify-center gap-2">
          <span className="h-3.5 w-3.5 rounded-full" style={{ background: g1.group_color }} />
          <p className="text-5xl font-black text-foreground">{g1.group_name}</p>
        </div>
        <p className="text-6xl font-black tabular-nums text-warning">
          {g1.total_points.toLocaleString('he-IL')}
        </p>
        <p className="mt-1 text-base text-muted">נקודות</p>
      </div>
      ...
      <div>
        <div className="mb-2 flex items-center justify-center gap-2">
          ...
          <p className="text-5xl font-black text-foreground">{g2.group_name}</p>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-surface-elevated px-6 py-3">
...
function RecentMissionsView({ transactions }: { transactions: TxRow[] }) {
  return (
    <div className="flex h-full flex-col gap-3">
      <p className="shrink-0 text-xs font-black uppercase tracking-widest text-muted">משימות אחרונות</p>
      <div className="flex flex-1 flex-col justify-between overflow-hidden">
        {transactions.slice(0, 6).map((tx, i) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate text-base font-black text-foreground">{tx.action?.name ?? '—'}</p>
              <p className="truncate text-sm text-muted">{tx.participant?.name ?? '—'}</p>
            </div>
            <span className="shrink-0 text-base font-black text-success">+{tx.points}</span>
          </motion.div>
        ))}
      </div>
    </div>
...
function AchievementsView({ rankedGroups, transactions }: { rankedGroups: RankedGroup[]; transactions: TxRow[] }) {
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      <p className="shrink-0 text-xs font-black uppercase tracking-widest text-muted">הישגים</p>
      <div className="flex flex-1 flex-col justify-between">
        {items.map((item, i) => (
          <motion.div
            ...
            className="flex items-center gap-4 rounded-2xl border border-border bg-surface-elevated px-5 py-4"
          >
            <span className="shrink-0 select-none text-4xl">{item.icon}</span>
            <div className="min-w-0">
              <p className="truncate text-lg font-black text-foreground">{item.title}</p>
    ...
    <div className="flex shrink-0 justify-center gap-1.5">
      {Array.from({ length: total }, (_, i) => (
        <motion.div
          ...
          style={{ height: 5, background: i === active ? `rgb(${r},${g},${b})` : 'var(--color-border)' }}
        />
      ))}
    </div>
  )
