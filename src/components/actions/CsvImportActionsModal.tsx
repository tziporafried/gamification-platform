...
export function CsvImportActionsModal({ eventId, isOpen, onClose, onImported }: CsvImportActionsModalProps) {
  ...
  async function handleImport() {
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        if (!rawName) {
          errors.push({ row: rowNumber, reason: 'Missing action name' })
          continue
        }

        if (!rawPoints) {
          errors.push({ row: rowNumber, reason: 'Missing points value' })
          continue
        }

        const pointsNum = parseInt(rawPoints, 10)
        if (isNaN(pointsNum) || rawPoints !== pointsNum.toString()) {
          errors.push({ row: rowNumber, reason: `Invalid points value: "${rawPoints}"` })
          continue
        }
        ...
        if (existingNames.has(nameLower)) {
          errors.push({ row: rowNumber, reason: `Action "${rawName}" already exists` })
          continue
        }

        if (seenNames.has(nameLower)) {
          errors.push({ row: rowNumber, reason: `Duplicate action "${rawName}" in CSV` })
          continue
        }
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Import Actions from CSV">
        ...
        {!summary ? (
          <>
            ...
            <div className="rounded-lg bg-gray-50 border border-gray-200 p-3">
              ...
              <code className="text-xs text-gray-600">action_name, points</code>
              <p className="text-xs text-gray-500 mt-2">
                Points can be positive or negative integers. Duplicate action names will be skipped.
              </p>
            </div>
          ...
          <>
            <div className="rounded-lg bg-gray-50 border border-gray-200 p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Actions created</span>
